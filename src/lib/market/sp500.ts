...
function parseCsvColumnFirstValue(line: string): string | null {
  ...
  return validSymbol(symbol) ? symbol : null;
}
...
export async function fetchSP500Symbols(): Promise<string[]> {
  const finnhubSymbols = await fetchFromFinnhub();
  ...
  const datahubSymbols = await fetchFromDataHub();
